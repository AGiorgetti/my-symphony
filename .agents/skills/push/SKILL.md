...
2. Run local validation (`dotnet format --verify-no-changes && dotnet build -c Release && dotnet test -c Release`) before pushing.
...
7. Validate PR body manually against `.github/pull_request_template.md` and ensure all sections are filled.
...
# Validate PR body against the template manually before submitting.
# Ensure all sections from .github/pull_request_template.md are filled
