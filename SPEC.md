...
(pluggable via tracker adapters), creates an isolated workspace for each issue, and runs a
...
- Issue tracker API (pluggable by `tracker.kind`).
...
- `kind` (string)
  ...
  - Supported values: `github`, `azure_devops`, and optionally `linear`.
  - Default built-in adapter order: `github`, `azure_devops`, then optional `linear`.
- `endpoint` (string)
  - Default for `tracker.kind == "linear"`: `https://api.linear.app/graphql`
  - Default for `tracker.kind == "github"`: `https://api.github.com`
  - Default for `tracker.kind == "azure_devops"`: `https://dev.azure.com`
- `api_key` (string)
  - May be a literal token or `$VAR_NAME`.
  - Canonical environment variable for `tracker.kind == "linear"`: `LINEAR_API_KEY`.
  - Canonical environment variable for `tracker.kind == "github"`: `GITHUB_TOKEN`.
  - Canonical environment variable for `tracker.kind == "azure_devops"`: `AZURE_DEVOPS_PAT`.
...
- `tracker.kind`: string, required, supports `github`, `azure_devops`, and optionally `linear`
- `tracker.endpoint`: string, default `https://api.github.com` when `tracker.kind=github`, `https://dev.azure.com` when `tracker.kind=azure_devops`, `https://api.linear.app/graphql` when `tracker.kind=linear`
- `tracker.api_key`: string or `$VAR`, canonical env `GITHUB_TOKEN` when `tracker.kind=github`, `AZURE_DEVOPS_PAT` when `tracker.kind=azure_devops`, `LINEAR_API_KEY` when `tracker.kind=linear`
- `tracker.project_slug`: string, required when `tracker.kind=linear`
- `tracker.repository`: string (`<owner>/<repo>`), required when `tracker.kind=github`
- `tracker.organization`: string, required when `tracker.kind=azure_devops`
...
## 11. Issue Tracker Integration Contract (Pluggable)

Tracker adapter model:

- Implementations must support pluggable tracker adapters selected by `tracker.kind`.
- Default built-in adapters should be provided in this order: `github`, `azure_devops`, then
  optional `linear`.
...
### 11.2 Query Semantics (Linear Adapter)
...
A non-Linear adapter may change transport details, but the normalized outputs must match the
domain model in Section 4.

### 11.3 Query Semantics (GitHub Adapter)
...
- `tracker.repository` must be `<owner>/<repo>`
- Candidate issue query filters by configured repository and active states
- Candidate fetch should exclude pull requests from issue results
- Pagination required for candidate issues
- Page size default: `50`
- Network timeout: `30000 ms`
...
- GitHub issue state is coarse (`open`/`closed`); adapter-level state mapping for
  `active_states`/`terminal_states` should be explicit and documented.

### 11.4 Query Semantics (Azure DevOps Adapter)

Azure DevOps-specific requirements for `tracker.kind == "azure_devops"`:

- `tracker.kind == "azure_devops"`
- REST endpoint default `https://dev.azure.com`
- Auth token sent via PAT-compatible authentication
- `tracker.organization` and `tracker.project` are required
- Candidate issue query filters by configured project and active states
- WIQL may be used to identify candidate IDs before batch work-item reads
- Pagination/batching required for candidate issues
- Page size default: `50`
- Network timeout: `30000 ms`

Important:

- Azure DevOps state/category names vary by process template; adapter-level mapping for
  `active_states`/`terminal_states` should be explicit and documented.
...
- For `tracker.kind == "azure_devops"`, `blocked_by` should be derived from work-item link
  relations that represent predecessors/dependencies blocking the current item.
- `priority` -> integer only (non-integers become null)
- For `tracker.kind == "github"`, `priority` is typically unavailable unless mapped from labels,
  and should be `null` when no explicit numeric mapping exists.
- For `tracker.kind == "azure_devops"`, numeric priority fields should be parsed as integers;
  non-numeric values should become `null`.
- `created_at` and `updated_at` -> parse ISO-8601 timestamps

#### 11.5.1 Example Field Mappings (Non-Normative)

The following examples are illustrative only; adapter implementations may use equivalent source
fields as long as normalized outputs match Section 4.1.1.

- Linear adapter examples:
  - `id` <- GraphQL `issue.id`
  - `identifier` <- GraphQL `issue.identifier`
  - `title` <- GraphQL `issue.title`
  - `description` <- GraphQL `issue.description`
  - `state` <- GraphQL `issue.state.name`
  - `created_at`/`updated_at` <- GraphQL `issue.createdAt`/`issue.updatedAt`
- GitHub adapter examples:
  - `id` <- REST `issue.id` (stringified)
  - `identifier` <- REST `issue.number` projected to a readable key (for example `#123`)
  - `title` <- REST `issue.title`
  - `description` <- REST `issue.body`
  - `state` <- REST `issue.state` or mapped workflow state label
  - `created_at`/`updated_at` <- REST `issue.created_at`/`issue.updated_at`
- Azure DevOps adapter examples:
  - `id` <- Work Item `id` (stringified)
  - `identifier` <- Work Item `id` projected to a readable key (for example `ADO-1234`)
  - `title` <- `System.Title`
  - `description` <- `System.Description`
  - `state` <- `System.State`
  - `created_at`/`updated_at` <- `System.CreatedDate`/`System.ChangedDate`
...
- `azure_devops_api_status` (non-200 HTTP)
...
1. `Workflow/Config Failures`
  ...
  - Unsupported tracker kind or missing tracker credentials/required project scope fields
...
- `tracker.kind` validation enforces supported kinds (`github`, `azure_devops`, optional `linear`)
...
- Temporary artifacts (e.g., `tmp`, `.vs`, `bin`, `obj`) are removed during prep
...
- GitHub candidate issue fetch scopes to configured `tracker.repository`
- Azure DevOps candidate issue fetch scopes to configured `tracker.organization` + `tracker.project`
- Empty `fetch_issues_by_states([])` returns empty without API call
- Pagination preserves order across multiple pages
- Blockers are normalized from inverse relations of type `blocks`
- GitHub blocker normalization maps dependency links when available and otherwise returns empty
- Azure DevOps blocker normalization maps predecessor/dependency link relations
- Labels are normalized to lowercase
- GitHub/Azure priority normalization returns integer when mappable, otherwise `null`
